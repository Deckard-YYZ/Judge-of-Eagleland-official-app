...
it("filters operation evidence and exposes export failure without invoking game work", async () => {
  ...
  fireEvent.change(screen.getByLabelText("筛选事件或 operation / Filter"), {
    target: { value: "op-1" },
  });
  expect(screen.getByText(/1 条记录/)).toBeTruthy();
  fireEvent.change(screen.getByLabelText("级别 / Level"), { target: { value: "error" } });
  expect(screen.getByText(/0 条记录/)).toBeTruthy();
  fireEvent.click(screen.getByText("详细日志 5 分钟 / Detailed"));
  expect(service.setDetailed).toHaveBeenCalledWith(true);
  fireEvent.click(screen.getByText("导出报告 / Export report"));
